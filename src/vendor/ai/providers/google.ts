		...
		try {
			...
			for await (const chunk of googleStream) {
				...
				if (candidate?.finishReason) {
					...
					// Capture error details if finish reason indicates an error
					if (output.stopReason === "error") {
						const errorDetails: string[] = [`finishReason: ${candidate.finishReason}`];
						if (candidate.finishMessage) {
							errorDetails.push(candidate.finishMessage);
						}
						if (candidate.safetyRatings) {
							const blocked = candidate.safetyRatings.filter((r: any) => r.blocked);
							if (blocked.length > 0) {
								errorDetails.push(`safety: ${blocked.map((r: any) => r.category).join(", ")}`);
							}
						}
						output.errorMessage = errorDetails.join("; ");
					}
				}
